...
class SentimentRecommenderModel:
    ...
    def __init__(self):
        ...
        self.model = pickle.load(open(os.path.join(SentimentRecommenderModel.ROOT_PATH , SentimentRecommenderModel.MODEL_NAME), 'rb'))

        self.vectorizer = pd.read_pickle(
           os.path.join(SentimentRecommenderModel.ROOT_PATH , SentimentRecommenderModel.VECTORIZER))
        
        self.user_final_rating = pickle.load(open(os.path.join(SentimentRecommenderModel.ROOT_PATH , SentimentRecommenderModel.RECOMMENDER), 'rb'))

        self.data = pd.read_csv("dataset/sample30.csv")

        self.cleaned_data = pd.read_pickle(os.path.join(SentimentRecommenderModel.ROOT_PATH , SentimentRecommenderModel.CLEANED_DATA))
